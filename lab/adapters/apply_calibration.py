...
class CalibrationApplicator:
    ...
    # Expected parameter locations in OmegaSportsAgent monorepo
    # All calibration lives in config/calibration/ (shared source of truth)
    PARAMETER_FILES = {
        'calibration_pack': 'config/calibration/nba_latest.json'
    }
    ...
    def __init__(self, agent_repo_path: Optional[str] = None):
        ...
        if not self.agent_repo_path.exists():
            logger.warning(f"⚠️  Agent repo not found: {self.agent_repo_path}")
        else:
    ...
    def load_calibration_pack(self, pack_path: str) -> Dict[str, Any]:
        ...
        logger.info(f"  Version: {pack.get('version')}")
        logger.info(f"  League: {pack.get('league')}")
        if pack.get("generated_at"):
            logger.info(f"  Generated: {pack.get('generated_at')}")
    ...
    def generate_patch_plan(
        ...
        plan = PatchPlan(
            calibration_version=pack.get('version', 'unknown'),
            league=pack.get('league', 'unknown'),
            changes=changes,
            notes=[
    ...
    def _detect_changes(self, pack: Dict[str, Any]) -> List[ParameterChange]:
        ...
        edge_thresholds = pack.get("edge_thresholds", {})
        kelly_policy = pack.get("kelly_policy", {})

        changes = []
        if "moneyline" in edge_thresholds:
            changes.append(
                ParameterChange(
                    file="config/thresholds.py",
                    parameter="MONEYLINE_EDGE_THRESHOLD",
                    old_value=0.02,
                    new_value=edge_thresholds.get("moneyline"),
                    line_number=15
                )
            )
        if "spread" in edge_thresholds:
            changes.append(
                ParameterChange(
                    file="config/thresholds.py",
                    parameter="SPREAD_EDGE_THRESHOLD",
                    old_value=0.03,
                    new_value=edge_thresholds.get("spread"),
                    line_number=16
                )
            )
        if "fraction" in kelly_policy:
            changes.append(
                ParameterChange(
                    file="config/staking.py",
                    parameter="KELLY_FRACTION",
                    old_value=0.25,
                    new_value=kelly_policy.get("fraction"),
                    line_number=22
                )
            )
