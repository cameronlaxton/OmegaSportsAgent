...
# Demo team context for default matchup (no network). Engine is input-driven.
_DEMO_CONTEXTS = {
    ("Lakers", "NBA"): {
        "name": "Lakers",
        "league": "NBA",
...
def _get_demo_context(home_team: str, away_team: str, league: str):
    """Return (home_context, away_context) for demo if available, else (None, None)."""
    if league.upper() != "NBA":
        return None, None
    home_ctx = _DEMO_CONTEXTS.get((home_team, "NBA"))
    away_ctx = _DEMO_CONTEXTS.get((away_team, "NBA"))
...
def run_example_simulation(
    ...
    home_ctx, away_ctx = _get_demo_context(home_team, away_team, league)
