...
Architecture: Agent-only for network calls. This module performs HTTP/API calls
(ESPN, Perplexity, Ball Don't Lie, etc.). Engine and service must not call
get_team_context/get_player_context; they receive pre-populated context from the caller.
"""
...
# Agent-layer config: used when agent calls this module for live data. Optional for core flows.
...
class TeamContext:
    ...
    Universal fields (used by all archetypes):
        name, league, off_rating, def_rating, pace

    Legacy basketball fields (kept for backward compat):
        pts_per_game, fg_pct, three_pt_pct

    Sport-specific extras go in ``stats`` dict so any archetype can pass
    additional keys without schema changes (e.g. ``stats["era"]`` for MLB,
    ``stats["xg_for"]`` for soccer, ``stats["serve_win_pct"]`` for tennis).
    """
    name: str
    league: str
    off_rating: float
    ...
    def to_dict(self) -> Dict[str, Any]:
        """Convert to dictionary, merging stats into top level."""
        d = asdict(self)
        extras = d.pop("stats", {})
        d.update(extras)
        return d
    ...
    def from_dict(cls, data: Dict[str, Any]) -> TeamContext:
        ...
        Known universal + basketball fields are extracted into dataclass attrs.
        Everything else flows into the ``stats`` catch-all dict so
        sport-specific keys (era, xg_for, serve_win_pct, ...) survive the
        round-trip.
        """
        _KNOWN_KEYS = {
            "name", "league", "off_rating", "def_rating", "pace",
            "pts_per_game", "fg_pct", "three_pt_pct", "stats",
        }
...
class PlayerContext:
    ...
    Universal fields: name, team, position
    Legacy basketball fields: usage_rate, pts_mean/std, reb_mean/std, ast_mean/std

    Sport-specific extras go in ``stats`` dict.  NFL players carry
    ``stats["pass_yards"]``, MLB ``stats["batting_avg"]``, etc.
    """
    name: str
    team: str
    position: str
    ...
    def from_dict(cls, data: Dict[str, Any]) -> PlayerContext:
        """Create from dictionary with sport-specific overflow into stats."""
        _KNOWN_KEYS = {
            "name", "team", "position", "usage_rate",
            "pts_mean", "pts_std", "reb_mean", "reb_std",
            "ast_mean", "ast_std", "stats",
        }
        extras = {k: v for k, v in data.items() if k not in _KNOWN_KEYS}
...
def get_team_context(team_name: str, league: str) -> Optional[TeamContext]:
    ...
    if context is None:
        logger.error(
            "DataIngestionError: no stats for team %s in any source (tried: %s)",
            team_name, sources_tried,
        )
        return None
...
def get_player_context(player_name: str, league: str) -> Optional[PlayerContext]:
    ...
    Fallback chain (agent-only): ESPN/BBRef -> Ball Don't Lie (NBA) -> Perplexity -> Last Known Good.
    Returns None if no data found in any source.
    ...
    if context is None:
        logger.error(
            "DataIngestionError: no stats for player %s in any source (tried: %s)",
            player_name, sources_tried,
        )
        return None
