...
Orchestrates data fetching with a fallback chain (ESPN -> BBRef -> NBA Stats API ->
Perplexity -> Last Known Good). Prefer stale real data over no data; log data source.

Architecture: Agent-only. Do not call from engine or service. Used when the agent
fetches data with fallback chain; engine/service receive pre-filled inputs only.
"""
...
class DataRecoveryService:
    """
    Data Recovery Service (agent-only).
    ...
    def get_team_stats(self, team_name: str, league: str) -> Optional[TeamContext]:
        """
        Try each data source in order (ESPN -> BBRef -> Perplexity -> Last Known Good).
        ...
        self.log_skipped_bet(
            entity=team_name,
            entity_type="team",
            reason="DataIngestionError: team has never been seen (no data in any source)",
            sources_tried=sources_tried
        )
    ...
    def get_player_stats(self, player_name: str, team: str, league: str) -> Optional[PlayerContext]:
        ...
        self.log_skipped_bet(
            entity=player_name,
            entity_type="player",
            reason="DataIngestionError: player has never been seen (no data in any source)",
            sources_tried=sources_tried
        )
